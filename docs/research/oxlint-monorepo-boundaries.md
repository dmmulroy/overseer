...
- Vite+ 0.2.7 owns lint configuration through the root [`vite.config.ts`](../../vite.config.ts). It already loads a local TypeScript JS plugin under the `overseer` alias and enables three custom rules. Vite+ documents that `vp lint` and `vp check` read this `lint` block and that root `lint.overrides` globs are workspace-root-relative ([Lint config](https://viteplus.dev/config/lint), [monorepo config](https://viteplus.dev/guide/monorepo#root-config-with-overrides)).
- The existing plugin uses `definePlugin`, `defineRule`, `context.filename`, AST visitors, and `createOnce` in [`tools/oxlint/overseer-option-plugin.ts`](../../tools/oxlint/overseer-option-plugin.ts). Its tests already use Oxlint's native `RuleTester` from `oxlint/plugins-dev` in [`tools/oxlint/overseer-option-plugin.test.ts`](../../tools/oxlint/overseer-option-plugin.test.ts).
