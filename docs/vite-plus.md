...
Cross-workspace infrastructure prerequisites belong in `dependsOn`. The API production deployment and both API E2E tasks depend on `@overseer/shared-infrastructure#deploy:production`, which owns the permanent Axiom trace datasets, least-privilege tokens, reusable Cloudflare Access policy, and R2 event data lake. Shared-infrastructure commands require the non-sensitive `OVERSEER_ACCESS_ALLOWED_EMAIL` deploy-time environment variable and Axiom provider credentials. The Cloudflare deployment principal must have Account API Tokens Write permission so Alchemy can create the scoped account-owned token used by R2 Data Catalog maintenance and Pipelines sinks.
