...
Cross-workspace infrastructure prerequisites belong in `dependsOn`. The API production deployment, deployed API E2E task, and production trace collector deployment depend on `@overseer/shared-infrastructure#deploy:production`, ensuring reusable Cloudflare Access policies and service credentials exist before an application Stack references them. Shared-infrastructure commands require the non-sensitive `OVERSEER_ACCESS_ALLOWED_EMAIL` deploy-time environment variable.
