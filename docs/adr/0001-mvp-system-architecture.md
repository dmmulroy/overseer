...
[#52](https://github.com/dmmulroy/overseer/issues/52) records a second contradiction. The REST contract partitions an idempotency key by a scope derived from the authenticated caller and uses the canonical target in its conflict fingerprint, while authoritative idempotency rows are partitioned between the Workspace Registry and each Project object. Independent objects cannot detect scope-global reuse. Coordination through the Workspace Registry would itself need an interruption and recovery protocol across objects. Slice 2 and later project-local ordinary POSTs await #52; Slice 1 operations owned only by the Workspace Registry can proceed.
...
- The settled ordinary key identity is idempotency scope plus key, retained for at least twenty-four hours. The scope derives from the authenticated caller. The fingerprint includes method, canonical target, normalized query, relevant metadata, and body; reuse on a different target conflicts.
- Workspace Registry and Project operations must use the ownership/recovery protocol selected by #52. This architecture does not weaken the public scope to per-object keys or silently introduce a cross-object reservation workflow.
