...
/** Canonical production API hostname or an isolated stage-qualified API hostname. */
export const OverseerApiHostnameSchema = Schema.String.check(
  Schema.isPattern(
    /^(?:api\.overseer\.mulroy\.ai|overseer-api-[a-z0-9](?:[a-z0-9-]{0,48}[a-z0-9])?\.mulroy\.ai)$/,
  ),
...
export const OverseerApiHostname = Alchemy.Stage.pipe(
  Effect.map((stage) => {
    ...
    return OverseerApiHostnameSchema.make(
      stage === "production" ? "api.overseer.mulroy.ai" : `overseer-api-${hostnameStage}.mulroy.ai`,
    );
  }),
);
