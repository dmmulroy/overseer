...
function getImportedName(specifier: ESTree.ImportSpecifier): string | null {
  if (specifier.imported.type === "Identifier") return specifier.imported.name;
  return specifier.imported.value;
}

/** Keep dependency-bearing service constructors local to their owning capability modules. */
  ...
  meta: {
    ...
    messages: {
      serviceConstructorImport:
        'Do not import service constructor "{{name}}" into runtime code. Import the owning Layer, yield the contextual service, and allow its requirements to propagate to the composition root.',
    },
  },
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        ...
        for (const specifier of node.specifiers) {
          ...
          if (importedName === null || !SERVICE_CONSTRUCTOR_NAME.test(importedName)) continue;
