...
`apps/api/src/api-worker.ts` exports `ApiWorker` as one inline Alchemy `Cloudflare.Worker` class. Its effectful props select the optional Access deployment and preserve generated Outputs directly in Worker environment bindings; its inline initialization Effect materializes the authenticated HTTP runtime. Keeping both phases in the bundled entry module gives Alchemy the Worker host context required by its Durable Object bindings without an empty class, `.make()` Layer, custom Access binding, or second Worker declaration. The Alchemy Stack yields `ApiWorker`, yields the same stable Access deployment Effect for output projection, and does not redeclare or wrap the Worker.
