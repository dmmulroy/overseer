...
describe("Effect v4 in workerd", () => {
  it("serves the shared HttpApi contract through Worker and Durable Object Fetch boundaries", async () => {
  ...
  it("authenticates and upgrades a Project-scoped WebSocket", async () => {
    const unauthorized = await SELF.fetch(
      "https://effect-spike.invalid/projects/project-spike/events",
    ...
    const upgraded = await SELF.fetch(
      "https://effect-spike.invalid/projects/project-spike/events",
      {
        headers: {
          upgrade: "websocket",
          "cf-access-authenticated-user-email": "human@example.test",
        },
      },
    );
    ...
    if (socket === null) {
      throw new Error("The successful upgrade response did not expose its WebSocket.");
    }
    socket.accept();
    const frame = await new Promise<unknown>((resolve, reject) => {
      socket.addEventListener("message", (event) => resolve(JSON.parse(String(event.data))));
      socket.addEventListener("error", () => reject(new Error("Project socket failed.")));
    });
